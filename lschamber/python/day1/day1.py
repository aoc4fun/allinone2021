...
#    CF part 1
def computeResult2(data):
    return sum(map(lambda entries: entries[0]>entries[1], zip(list(map(sum, zip(data[:-2],data[1:-1], data[2:])))[1:], list(map(sum, zip(data[:-2],data[1:-1], data[2:])))[0:-1])))

def test(name, function, data, expected):
    print(f"{name} - {'Success' if expected == function(data) else 'Failure'} : expected '{expected}' got '{function(data)}'")

def part1(data):
    print(f"Part 1: {computeResult1(data)}")

def part2(data):
    print(f"Part 2: {computeResult2(data)}")

def main():
    with open(TEST_FILE, "r") as testFile:
        testData = [int(x) for x in testFile.readlines()]
        test("Test 1", computeResult1, testData, 7)
        test("Test 2", computeResult2, testData, 5)
